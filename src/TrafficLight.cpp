...
void MessageQueue<T>::send(T &&msg)
{
    ...
    _queue.clear();  // only latest message is relevant 
...
void TrafficLight::cycleThroughPhases()
{
    ...
    double cycleDurationMin = 4.0 * 1000;  // in ms 
    double cycleDurationMax = 6.0 * 1000; 
    double currCycleDuration = getRandomCycleTime(cycleDurationMin, cycleDurationMax); 
...
double TrafficLight::getRandomCycleTime(double minCycleTime, double maxCycleTime){
    std::random_device rd;
    static std::mt19937 eng(rd());
    static std::uniform_real_distribution<double> unif(minCycleTime, maxCycleTime); 
    double randomValue = unif(eng); 
    return randomValue; 
}
